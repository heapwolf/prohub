    ...
    milestonesChart = c3.generate({
      ...
      data: {
        ...
        onmouseover: function(d) {
          var id = d.id;
          var tid = 'tip-' + id.replace(/\//g,'-').replace(/ /g, '-').replace('-Open', '').replace('-Closed', '');
...
exports.datebar = function(data, filters) {
  ...
  calculated.dates.milestones.forEach(function(m) {
    var title = m[0] + ' ' + new Date(m[1]).getMonth()+1 + '/' + new Date(m[1]).getDate() + '/' + new Date(m[1]).getFullYear();
    var id = m[2].replace(/\//g,'-').replace(/ /g, '-');
    createTip(title, id, 'tooltip', m[1], true);
  });

  createTip('Today', 'today', 'tooltip', calculated.dates.today)
  createTip(
    'Start ' + new Date(calculated.dates.earliest).getMonth()+1 + '/' + new Date(calculated.dates.earliest).getFullYear(),
    'start', 'tooltip-bottom tooltip-start', calculated.dates.earliest);
  createTip(
    'End ' + new Date(calculated.dates.latest).getMonth()+1 + '/' + new Date(calculated.dates.latest).getDate() + '/' + new Date(calculated.dates.latest).getFullYear(),
    'end', 'tooltip-bottom tooltip-end', calculated.dates.latest);
