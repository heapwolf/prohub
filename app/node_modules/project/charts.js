...
function formatId(id) {
  return id.replace(/[\/,.\-_]+/g,'-').replace(/ /g, '-').replace('-Open', '').replace('-Closed', '').replace(/[\-]{2,}/g, '-').toLowerCase();
}
    ...
    milestonesChart = c3.generate({
      ...
      data: {
        ...
        onmouseover: function(d) {
          ...
          d3.select('[data-id="' + tid + '"]').classed({hidden: false});
          d3.select('[data-id="tip-today"]').classed({hidden: true});
        },
        onmouseout: function(d) {
          var id = d.id;
          var tid = 'tip-' + formatId(id);
          d3.select('[data-id="' + tid + '"]').classed({hidden: true});
          d3.select('[data-id="tip-today"]').classed({hidden: false});
        }
      },
      donut: {
        title: ''
...
exports.datebar = function(data, filters) {
  ...
  function createTip(text, id, classes, date, hidden) {
    ...
    d3.select('[data-id="' + id + '"]').style('left', left+'px').classed({hidden: hidden});
  }
